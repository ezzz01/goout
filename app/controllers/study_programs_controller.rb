  ...
  def create
    ...
    respond_to do |format|
      if @study_program.save
        ...
        format.js {
            subject_area = SubjectArea.find(params[:study_program][:subject_area_id], :include => :study_programs, :order => 'concepts.title', :conditions => [ "concepts.pending = 0 OR concepts.added_by = ?", current_user.id ])
